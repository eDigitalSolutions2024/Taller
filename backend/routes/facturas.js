...
router.get('/facturas-proveedor', async (req, res) => {
  try {
    ...
    const pipeline = [
      ...
      // No agrupamos: cada entrada es una fila independiente, así conservamos
      // el _id real de MongoDB para los botones de acción (Ver/Editar/Continuar).
      { $sort: sortStage },
        ...
        $addFields: {
          proveedorNombre: {
            $ifNull: [
              ...
              { $ifNull: [{ $arrayElemAt: ['$provInfo.nombre', 0] }, 'Sin nombre'] },
            ],
          },
        },
      },
