...
const SEQ_NOTA_VENTA = 'pago_notaVenta';
const SEQ_REMISION = 'pago_remision';
const SEQ_RECIBO_PROVISIONAL = 'pago_reciboProvisional';
const SEQ_RECIBO_DOLARES = 'pago_reciboDolares';
...
// GET /api/cajas -> lista de órdenes para el módulo de Cajas. Un cobro puede
// llegar en cualquier etapa; solo se ocultan canceladas y ya liquidadas.
// vista=activas (default) | cerradas | liquidadas | pendientes | garantias
// sort=recientes (default) | os_asc | os_desc
...
function compararOrdenServicio(a, b) {
  const partes = (os) => {
    ...
    return m
      ? { letras: m[1].toUpperCase(), numero: parseInt(m[2], 10) }
      : { letras: String(os || '').toUpperCase(), numero: 0 };
  };
  const pa = partes(a);
  const pb = partes(b);
...
router.get('/', proteger, async (req, res) => {
  try {
    ...
    const ordenes = await Vehiculo.find(q).sort({ createdAt: -1 }).populate('cliente', 'esEmpleado');

    // El saldo pendiente no se persiste (se recalcula siempre), así que
    // decidir si una orden ya está "liquidada" obliga a calcularlo aquí y
    // paginar en memoria en vez de con skip/limit.
...
// su propio comprobante con folio nuevo asignado en el momento.
router.post('/:id/pagos', proteger, async (req, res) => {
  try {
    ...
    if (tipoPago === 'COMPLETO' && comprobante === 'RECIBO_PROVISIONAL') {
      return res.status(400).json({ ok: false, msg: 'Un pago que liquida requiere Nota de Venta o Remisión.' });
    }
    ...
    // Remisión ni una Nota de Venta. Una remisión cancelada no cuenta.
    ...
    // pago que puede registrarse en 0 (queda como cuenta por cobrar).
    ...
    if (comprobante === 'NOTA_VENTA') {
      pago.notaVenta = { numero: await siguienteFolio(SEQ_NOTA_VENTA), banco, tipo: tipoNota };
      ...
      pago.remision = { numero: await siguienteFolio(SEQ_REMISION), tipo: tipoRemision, fechaPagada: null };
    }

    // Recibo Provisional: automático en cada abono/anticipo.
    if (['ABONO', 'ANTICIPO'].includes(tipoPago)) {
      pago.reciboProvisional = {
        numero: await siguienteFolio(SEQ_RECIBO_PROVISIONAL),
        formaPago,
    ...
    if (Number(montoDolares) > 0) {
      pago.reciboDolares = { numero: await siguienteFolio(SEQ_RECIBO_DOLARES) };
    }
...
// El pago no se borra (su folio ya se consumió): queda marcado como cancelado
// y deja de contar como abonado.
router.post('/:id/pagos/:pagoId/cancelar', proteger, requiereRol('admin'), async (req, res) => {
  try {
    ...
    pago.canceladoPor = req.user?.name || '';
...
// PUT /api/cajas/:id/descuentos/:descuentoId -> edita o activa/desactiva un descuento
...
// ===== PDFs =====
// Sin `proteger`: se abren vía window.open() y ese request no puede llevar el
// header Authorization, igual que los PDFs en routes/vehiculos.js.
// ?pagoId= identifica cuál pago (con su comprobante) imprimir; sin él se
...
// Recibo Provisional / Recibo de Dólares: dependen de si el pago trae
// reciboProvisional/reciboDolares asignado (ver POST /:id/pagos).
