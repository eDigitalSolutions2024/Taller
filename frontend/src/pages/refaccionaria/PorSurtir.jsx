...
export default function PorSurtir() {
  ...
  const cargar = async () => {
    try {
      ...
      const todas = [...(r1.data?.data || []), ...(r2.data?.data || [])].sort((a, b) => {
  ...
  const nombreAsesor = (orden) => orden.creadoPor || "Sin asesor";
  ...
  const aplicarDetalle = (ordenId, presupuestoIndex, detalle) => {
    setOrdenes((prev) =>
      prev.map((o) => {
        ...
        const presupuesto = o.presupuesto.map((p, i) => (i !== presupuestoIndex ? p : { ...p, ...detalle }));
  ...
  const cambiarCantidad = (ordenId, realIdx, value) => {
    ...
    setOrdenes((prev) =>
      prev.map((o) => {
        if (o._id !== ordenId) return o;
        const presupuesto = o.presupuesto.map((p, i) => (i !== realIdx ? p : { ...p, cant: Number.isFinite(cant) ? cant : p.cant }));
        return { ...o, presupuesto };
      })
    );
  };

  const ordenModalDetalle = modalDetalle ? ordenes.find((o) => o._id === modalDetalle.ordenId) : null;
  ...
  const handleGuardar = async (orden) => {
    ...
    const updatedPresupuesto = orden.presupuesto.map((p, i) => (selected.has(i) ? { ...p, surtida: true } : p));
    ...
    <div className="container-fluid py-3">
      <div className="card">
        ...
        <div className="card-body">
          {ordenes.length === 0 ? (
            <div className="alert alert-info text-center">No hay refacciones pendientes de surtir.</div>
          ) : (
            ordenes.map((orden) => {
              // Las partidas de servicio no requieren surtido.
              const autorizadas = (orden.presupuesto || []).filter((p) => p.autorizado && !p.surtida && !p.esServicio);
                ...
                <div key={orden._id} className="card mb-3">
                  <div className="card-header d-flex justify-content-between align-items-center">
                    <div className="flex-grow-1 me-3">
                      ...
                      {orden.devueltoPor && (
                        <span className="ms-3 badge bg-info text-dark">👤 Devuelto por: {orden.devueltoPor}</span>
                      )}
                    </div>
                    <button
                      type="button"
                    ...
                    <table className="table table-bordered table-sm align-middle mb-0">
                      ...
                      <tbody>
                        {autorizadas.map((p) => {
                          ...
                          return (
                            <tr key={realIdx} className={isSelected ? "table-success" : tiempo.rowClassName}>
          ...
          <button type="button" className="btn btn-outline-secondary btn-sm mt-2" onClick={cargar} disabled={loading}>
            Actualizar
          </button>
        </div>
      </div>
      ...
      {modalDetalle && filaModalDetalle && (
        <ModalCotizarSurtido
          ...
          onGuardar={(detalle) => aplicarDetalle(modalDetalle.ordenId, modalDetalle.presupuestoIndex, detalle)}
        />
      )}
    </div>
  );
