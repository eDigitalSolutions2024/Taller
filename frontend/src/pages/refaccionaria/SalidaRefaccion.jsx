...
export default function SalidaInventario() {
  ...
  const totalRows = useMemo(() => rows.reduce((a,r)=>a+Number(r.cantidad||0),0), [rows]);
  ...
  async function resolverUnidad(codigoInterno) {
    try {
      const r = await fetch(`${API}/inventario/${encodeURIComponent(codigoInterno)}/unidad`, {
        credentials: "include"
      });
      const j = await r.json().catch(()=>({}));
  ...
  async function agregar() {
    ...
    const unidad = await resolverUnidad(codigoSel); // << se trae de la BD
    ...
    setRows(prev => [
      ...prev,
      {
        ...
        marca: marca || "",
        unidad,                               // << guardamos unidad resuelta
  ...
  async function finalizar() {
    if (rows.length === 0) return alert("Agrega al menos una refacción.");
    try {
      const payload = {
        fechaSalida: fecha,
        ordenServicio: os || "",
        partidas: rows.map(r => ({
          ...
          marca: r.marca,
          unidad: r.unidad,                    // << ya viene rellena
          cantidad: r.cantidad,
        })),
      };
          ...
          <div className="card shadow-sm border-0">
            ...
            <div className="card-body">
              <div className="row g-3">
                ...
                <div className="col-md-3">
                  ...
                  <input className="form-control" value={os} onChange={e=>setOs(e.target.value)} />
                </div>

                <div className="col-md-2">
                  <label className="form-label">Cantidad:</label>
                  <input type="number" min="0" step="any" className="form-control" value={cantidad}
                         onChange={e=>setCantidad(e.target.value)} />
                </div>

                <div className="col-md-4">
                  <label className="form-label">Refacción:</label>
                  <select className="form-select" value={codigoSel} onChange={e=>setCodigoSel(e.target.value)}>
                    <option value="">Select an Option</option>
                    {catalogo.map(o => <option key={o._id} value={o._id}>{o.label}</option>)}
                  </select>
                </div>
              </div>
            ...
            <div className="table-responsive px-3">
              <table className="table table-bordered align-middle">
                ...
                <tbody>
                  {rows.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-4">Sin partidas</td></tr>
                  ) : rows.map((r, i) => (
                    <tr key={i}>
                      <td>{r.cantidad}</td>
                      <td>{r.unidad}</td> {/* << solo se muestra, no se edita */}
                      <td>
                        <div className="fw-semibold">{r.codigoLabel}</div>
                        <div className="small text-muted">{r.descripcion}</div>
                      </td>
                      <td>{r.marca}</td>
                      <td>{r.os || os}</td>
                      <td>
                        <button className="btn btn-danger btn-sm" onClick={()=>borrar(i)}>Borrar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="d-flex justify-content-end pb-3">
                <button className="btn btn-primary" onClick={finalizar}>Finalizar Captura</button>
              </div>
            </div>
