    ...
    <>
      ...
      <div
        style={{
          ...
          background: "white", borderRadius: 8, boxShadow: "0 8px 32px rgba(0,0,0,0.2)", padding: 24,
        }}
      >
        ...
        <div className="alert alert-warning py-2 mb-3 small">
          ...
          <strong>Fecha:</strong> {fmtFecha(entrada.fecha)}
        </div>

        <div className="mb-3">
          <label className="form-label">Selecciona imagen o PDF</label>
          <input type="file" className="form-control" accept="image/*,application/pdf" onChange={onFile} />
        ...
        {preview && (
          <div className="border rounded p-2 mb-3">
            {tipo === "application/pdf" ? (
              <iframe src={preview} title="Vista previa" width="100%" height="220px" style={{ border: "none" }} />
            ) : (
              <>
                <img
                  src={preview}
                  ...
                  style={{ width: "100%", maxHeight: 220, objectFit: "contain", cursor: "zoom-in", borderRadius: 4 }}
                />
      ...
      {zoomAbierto && (
        <div
          ...
          style={{
            position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.88)", zIndex: 2000,
            display: "flex", alignItems: "center", justifyContent: "center", cursor: "zoom-out",
          }}
        >
          <img src={preview} alt="Zoom" style={{ maxWidth: "90vw", maxHeight: "90vh", objectFit: "contain", borderRadius: 8, boxShadow: "0 0 40px rgba(0,0,0,0.6)" }} />
          <button
            onClick={() => setZoomAbierto(false)}
        ...
        <div style={{ overflowY: "auto", padding: 24 }}>
          <div className="row g-3 mb-4">
            ...
            <div className="col-6 col-md-4">
              ...
              <strong>{fmtFecha(entrada.fechaFactura)}</strong>
            </div>
            <div className="col-6 col-md-4">
            ...
            <div className="mb-4">
              ...
              {entrada.fotoFactura.mimetype === "application/pdf" ? (
                <a
                  ...
                  href={`${SERVER}${entrada.fotoFactura.url}`}
                  target="_blank"
                ...
                <img
                  ...
                  style={{ maxWidth: "100%", maxHeight: 400, objectFit: "contain", border: "1px solid #dee2e6", borderRadius: 4, display: "block" }}
                />
              )}
            </div>
          )}
...
function ModalEditarEntrada({ entrada, onClose, onGuardado }) {
  ...
  const handleCodigoSeleccionado = (item) => {
    setCaptura((prev) => {
      const c = [...prev];
      c[modalCodigoIdx] = {
        ...c[modalCodigoIdx],
        codigoInterno: item._id,
        descripcion: item.nombrePieza || c[modalCodigoIdx].descripcion,
        marca: item.marca || c[modalCodigoIdx].marca,
        unidad: item.unidadMedida || c[modalCodigoIdx].unidad,
        costoUnitario:
  ...
  const addFila = () =>
    setCaptura((prev) => [
      ...prev,
      { descripcion: "", tipo: "Refacción", unidad: "", cantidad: 0, costoUnitario: 0, ivaPct: 8, costoDescuento: 0, marca: "" },
    ]);
  ...
  return (
    <>
      ...
      >
        <div className="d-flex justify-content-between align-items-center p-3 border-bottom">
          <h5 className="mb-0">
            Editar Factura <span className="text-muted small">#{entrada.numero || entrada._id}</span>
          </h5>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", lineHeight: 1 }}>×</button>
        </div>
        ...
        <div style={{ overflowY: "auto", padding: 24 }}>
          <div className="row g-3 mb-4">
            ...
            <div className="col-md-3">
              ...
              <select className="form-select" name="formaPago" value={form.formaPago} onChange={onF}>
                <option>Crédito</option><option>Contado</option><option>Transferencia</option><option>Efectivo</option><option>Tarjeta</option>
              </select>
            </div>
          </div>

          <h6 className="text-uppercase text-muted mb-2">Renglones de captura</h6>
          <div className="table-responsive mb-2">
            <table className="table table-sm table-bordered align-middle">
              <thead className="table-light">
                <tr>
                  <th style={{ minWidth: 220 }}>Código / Descripción</th>
                  <th>Tipo</th>
                  <th>Unidad</th>
                  <th style={{ width: 90 }}>Cantidad</th>
                  <th style={{ width: 120 }}>Costo Unitario</th>
                  ...
                  <tr key={i}>
                    <td>
                      <div className="d-flex gap-1">
                        <input
                          ...
                          value={c.descripcion}
                          onChange={(e) => onCaptura(i, "descripcion", e.target.value)}
                        />
                        <button type="button" className="btn btn-sm btn-outline-secondary text-nowrap" onClick={() => setModalCodigoIdx(i)}>
                          🔍
                        </button>
                      </div>
                    </td>
                    <td>
                      <select className="form-select form-select-sm" value={c.tipo} onChange={(e) => onCaptura(i, "tipo", e.target.value)}>
                        <option>Refacción</option><option>Insumo</option><option>Servicio</option>
                      </select>
                    </td>
                    <td>
                      <input className="form-control form-control-sm" value={c.unidad} onChange={(e) => onCaptura(i, "unidad", e.target.value)} />
                    </td>
                    <td>
                      <input type="number" min="0" className="form-control form-control-sm" value={c.cantidad}
                        onChange={(e) => onCaptura(i, "cantidad", Number(e.target.value))} />
                    </td>
                    <td>
                      <input type="number" min="0" step="0.01" className="form-control form-control-sm" value={c.costoUnitario}
                        onChange={(e) => onCaptura(i, "costoUnitario", Number(e.target.value))} />
                    </td>
                    <td>
                      <select className="form-select form-select-sm" value={c.ivaPct} onChange={(e) => onCaptura(i, "ivaPct", Number(e.target.value))}>
                        <option value={0}>0%</option><option value={8}>8%</option><option value={16}>16%</option>
                      </select>
                    </td>
                    <td>
                      <input type="number" min="0" step="0.01" className="form-control form-control-sm" value={c.costoDescuento}
                        onChange={(e) => onCaptura(i, "costoDescuento", Number(e.target.value))} />
                    </td>
                    <td className="text-end fw-semibold">{formatCurrency(calcConIva(c))}</td>
                    <td>
                      <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => removeFila(i)}>✕</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  <td colSpan={7} className="text-end fw-semibold">Total General:</td>
                  <td className="fw-bold">{formatCurrency(totalGeneral)}</td>
                  <td />
                </tr>
              </tfoot>
          ...
          <button type="button" className="btn btn-outline-primary btn-sm" onClick={addFila}>+ Agregar renglón</button>
        </div>
      ...
      {modalCodigoIdx !== null && (
        <ModalBuscarCodigo onSeleccionar={handleCodigoSeleccionado} onCerrar={() => setModalCodigoIdx(null)} />
      )}
    </>
  );
}
...
export default function ConsultarFacturaProveedor() {
  ...
  const [f, setF] = useState({ numero: "", q: "", proveedor: "", estado: "todos", desde: "", hasta: "" });
  const onF = (e) => setF((s) => ({ ...s, [e.target.name]: e.target.value }));

  const fetchData = async (_page = page, _limit = limit) => {
    setLoading(true);
    try {
      const { data } = await http.get("/facturas-proveedor", { params: { ...f, page: _page, limit: _limit } });
      if (data?.ok) {
        setRows(data.docs || []);
        setPage(data.page);
        setLimit(data.limit);
  ...
  const eliminarBorrador = async (row) => {
    ...
    try {
      const r = await fetch(`${API}/entradas/${row._id}`, { method: "DELETE", credentials: "include" });
      const json = await r.json().catch(() => ({}));
  ...
  const onFotoGuardada = (id) => {
    setRows((prev) => prev.map((r) => (r._id === id ? { ...r, fotoFactura: { url: "actualizada" } } : r)));
  };
        ...
        <div className="card-body">
          <div className="row g-3 align-items-end">
            ...
            <div className="col-md-3">
              ...
              <input className="form-control" name="q" value={f.q} onChange={onF} placeholder="texto libre" />
            </div>
            <div className="col-md-1">
              ...
              <select
                ...
                value={limit}
                onChange={(e) => { const l = parseInt(e.target.value, 10) || 10; setLimit(l); fetchData(1, l); }}
              >
      ...
      <div className="card shadow-sm">
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r._id} style={sinFoto ? { backgroundColor: "#fff3e0" } : {}}>
                    <td>
                      {sinFoto ? (
                        <span
                          ...
                          style={{ cursor: "pointer", color: "#e65100", fontWeight: 600, borderBottom: "2px dashed #e65100" }}
                        >
                          {r.factura}
                        </span>
                      ) : (
                    ...
                    <td>{r.estado === "finalizada" ? "🟢 Finalizada" : "🟡 Borrador"}</td>
                    <td>
                      <div className="d-flex flex-wrap gap-1">
                        {r.estado === "borrador" && (
                          <button className="btn btn-warning btn-sm" onClick={() => continuar(r)}>Continuar</button>
                        )}
                        <button className="btn btn-outline-info btn-sm" onClick={() => verDetalle(r)}>Ver info</button>
                        {esAdmin && (
                          <button className="btn btn-outline-warning btn-sm" onClick={() => abrirEditar(r)}>Editar</button>
                        )}
                        {r.estado === "borrador" && (
                          <button className="btn btn-outline-danger btn-sm" onClick={() => eliminarBorrador(r)}>Eliminar</button>
                        )}
                        {r.estado === "finalizada" && (
        ...
        <div className="d-flex justify-content-between align-items-center p-2">
          ...
          <div className="btn-group">
            <button className="btn btn-outline-secondary" disabled={page <= 1 || loading} onClick={() => fetchData(page - 1, limit)}>Previous</button>
            <span className="btn btn-outline-secondary disabled">{page}</span>
            <button className="btn btn-outline-secondary" disabled={page >= totalPages || loading} onClick={() => fetchData(page + 1, limit)}>Next</button>
          </div>
        </div>
      </div>

      {entradaFoto && (
        <ModalSubirFoto entrada={entradaFoto} onClose={() => setEntradaFoto(null)} onFotoGuardada={onFotoGuardada} />
      )}

      {entradaDetalle && (
        <ModalVerDetalle key={entradaDetalle._id} entrada={entradaDetalle} onClose={() => setEntradaDetalle(null)} />
      )}

      {entradaEditar && (
        <ModalEditarEntrada entrada={entradaEditar} onClose={() => setEntradaEditar(null)} onGuardado={onEntradaGuardada} />
      )}
    </div>
  );
}
