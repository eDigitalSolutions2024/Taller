...
export default function VehiculoEntrada() {
  ...
  useEffect(() => {
    ...
    const resultado = clientes.filter((c) => {
      ...
      const nombreFiltro = c.gobierno?.nombreGobierno || nombreCompleto || "";
    ...
    <div className="container-fluid">
        ...
        <div className="card-body">
          ...
          {!loading && !error && q.trim().length > 0 && filtrados.length === 0 && (
            <div className="mt-3 d-flex align-items-center gap-3">
              <p className="text-muted mb-0">No se encontraron clientes con ese nombre.</p>
              <button
                type="button"
          ...
          {mostrarAcciones && clienteSeleccionado && (
            <div className="mt-3">
              ...
              <button type="button" className="btn btn-primary me-2" onClick={handleNuevoCarro}>
                Nuevo Carro
              </button>
              <button type="button" className="btn btn-secondary me-2" onClick={handleSinVehiculo}>
                Sin Vehículo
              </button>
              <button type="button" className="btn btn-success me-2" onClick={handleGaraje}>
                Garaje
              </button>
              <button type="button" className="btn btn-warning" onClick={handleGarantia}>
                Garantía
              </button>
            </div>
          )}
      ...
      {mostrarFormNuevoCarro && clienteSeleccionado && garantiaInfo && (
        <div className="alert alert-warning mt-3 mb-0">
          ...
          <strong>{garantiaInfo.ordenAnterior?.ordenServicio}</strong>. Motivo: {garantiaInfo.motivo}
        </div>
      )}
      ...
      {mostrarFormNuevoCarro && clienteSeleccionado && (
        <VehiculoNuevoForm
          ...
          prefillVehiculo={vehiculoGarage}
          garantiaSolicitud={
            garantiaInfo
              ? { ordenAnteriorId: garantiaInfo.ordenAnterior?._id, motivo: garantiaInfo.motivo }
              : null
          }
      ...
      <small className="text-muted d-block mt-2">
        * Selecciona un cliente de la lista para continuar con el registro del vehículo o de la
        orden sin carro.
      </small>
