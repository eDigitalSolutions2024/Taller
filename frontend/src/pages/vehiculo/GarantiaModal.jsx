...
// SERIE del vehículo o por número de orden. Al confirmar se continúa el flujo
// de nueva orden con el vehículo prellenado.
export default function GarantiaModal({ show, cliente, onSolicitar, onClose }) {
  ...
  const norm = (valor) => String(valor || "").toUpperCase().replace(/[-\s]/g, "");

  // Las órdenes de Taller guardan el nombre del cliente directamente en la
  // propia orden (snapshot plano), no hay que resolverlo desde un populate.
  const nombreOrden = (o) =>
    o?.nombreGobierno ||
  ...
  const ordenesFiltradas = termino
    ? ordenesCliente.filter((o) => norm(o.serie).includes(termino) || norm(o.ordenServicio).includes(termino))
    : ordenesCliente;
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      const cerradas = [...porId.values()].filter(
        (o) => o.estadoOrden === "CERRADA" && (norm(o.serie).includes(t) || norm(o.ordenServicio).includes(t))
      );
      ...
      if (!disponibles.length) {
        setError(`Las órdenes cerradas que coinciden con "${term}" ya fueron utilizadas en una garantía.`);
        return;
      }
          ...
          <div className="modal-body">
            ...
            <div className="input-group mb-2">
              <input
                ...
                placeholder="Filtrar por serie o número de orden (Ej. OS-00012)"
                value={serie}
                autoFocus
              ...
              <button
                ...
                disabled={buscando}
                title="Busca la serie o el número de orden en todas las órdenes"
              >
            ...
            <div className="mb-2">
              <small className="text-muted">
                Órdenes cerradas de <strong>{nombreOrden(cliente?.tipoCliente === "Particular" ? { nombreCliente: cliente.nombre, apellidoPaterno: cliente.apellidoPaterno, apellidoMaterno: cliente.apellidoMaterno } : { nombreGobierno: cliente?.gobierno?.nombreGobierno || cliente?.nombre })}</strong>
                {cliente?.esEmpleado && (
                ...
                return (
                  ...
                  >
                    <div className="d-flex justify-content-between align-items-center">
                      <span className="fw-bold">Serie: {o.serie || "—"}</span>
