...
export default function VehiculoReparacionEnCurso({ orden, onSaved, onGoGeneral, readOnly = false }) {
  ...
  const otrosServicios = catalogo.filter((s) => codigosSeleccionados.includes(s.codigo));
  ...
  const subtotalVenta = ventaCliente.reduce((s, r) => s + Number(r.cant || 0) * Number(r.precioVenta || 0), 0);
  ...
  const tieneGrua = orden.grua === "SI";
  const precioGrua = Number(orden.precioGrua || 0);
  const gruaEnVenta = ventaCliente.some((r) => r.esGrua);

  // manoObra.mecanico / manoObra.carrocero ya guardan el nombre directamente
  // (no un id que haya que resolver contra el catálogo de empleados).
  const personalRows = manoObra
    .map((m) => ({
      rol: m.esCarroceria ? "Carrocero" : "Mecánico",
      nombre: (m.esCarroceria ? m.carrocero : m.mecanico) || "—",
      concepto: m.concepto || "—",
  ...
  return (
    <div className="card card-body mb-4">
      ...
      <div className="border rounded p-3 mb-4 bg-light" style={{ minHeight: "60px", whiteSpace: "pre-wrap" }}>
        {orden.diagnosticoTecnico ? orden.diagnosticoTecnico : <span className="text-muted">Sin diagnóstico registrado.</span>}
      </div>
        ...
        <table className="table table-sm table-bordered align-middle">
          ...
          <tbody>
            {refacciones.length === 0 && (
              <tr>
                <td colSpan={8} className="text-center text-muted">Sin refacciones autorizadas.</td>
              </tr>
            )}
            {refacciones.map((p, i) => (
        ...
        <table className="table table-sm table-bordered align-middle">
          ...
          <tbody>
            {ventaCliente.length === 0 && (
              <tr>
                <td colSpan={4} className="text-center text-muted">Sin partidas de venta al cliente.</td>
              </tr>
            )}
      ...
      {!readOnly && (
        <div className="d-flex justify-content-end align-items-center gap-3 mb-4">
          {!todasSurtidas && (
            <span className="text-danger small fw-semibold">Hay refacciones pendientes de surtir</span>
          )}
          <button
            type="button"
