// src/pages/vehiculo/GarageAdminPage.jsx
...
// El snapshot del cliente de cada orden cerrada vive plano en la propia orden.
function nombreClienteOrden(o) {
  return (
    o.nombreGobierno ||
    [o.nombreCliente, o.apellidoPaterno, o.apellidoMaterno].filter(Boolean).join(" ") ||
    "Sin nombre"
  );
}
...
export default function GarageAdminPage() {
  ...
  const handleImportar = async () => {
    ...
    try {
      ...
      alert(msg || "Importación completada.");
      cargar();
    } catch (err) {
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <p className="text-muted mb-3">Catálogo de vehículos registrados por número de serie.</p>
            ...
            <div key={v._id} className="card border shadow-sm">
              ...
              >
                <div className="d-flex align-items-center gap-3 flex-wrap">
                  <strong className="fs-6">{[v.marca, v.modelo].filter(Boolean).join(" ") || "Vehículo sin nombre"}</strong>
                  {v.anio && (
                    <span className="text-muted" style={{ fontSize: "0.9rem" }}>
                      {v.anio}{v.color ? ` · ${v.color}` : ""}
                    </span>
                  )}
                  <span className="badge bg-light text-dark border" style={{ fontSize: "0.8rem" }}>Serie: {v.serie}</span>
                  {v.placas && <span className="badge bg-light text-dark border" style={{ fontSize: "0.8rem" }}>Placas: {v.placas}</span>}
                </div>
                <div className="d-flex align-items-center gap-2">
                  <span className="badge bg-primary">
                    {ordenes.length} orden{ordenes.length !== 1 ? "es" : ""} cerrada{ordenes.length !== 1 ? "s" : ""}
                  </span>
                  <span className="text-muted" style={{ fontSize: "1.1rem" }}>{abierto ? "▲" : "▼"}</span>
                </div>
              </div>
              ...
              {abierto && (
                <div className="card-body">
                  <div className="row g-4">
                    ...
                    <div className="col-12 col-md-4">
                      <h6 className="fw-semibold border-bottom pb-1 mb-2">Clientes ({clientes.length})</h6>
                      {clientes.length === 0 ? (
                        <p className="text-muted" style={{ fontSize: "0.88rem" }}>Sin clientes registrados</p>
                      ) : (
                        <ul className="list-group list-group-flush">
                          {clientes.map((c) => (
                            <li key={c._id} className="list-group-item px-0 py-1" style={{ fontSize: "0.88rem" }}>
                              {getNombreCliente(c)}
                              {c.esEmpleado && <span className="badge bg-warning text-dark ms-1">Empleado</span>}
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <div className="col-12 col-md-4">
                      <h6 className="fw-semibold border-bottom pb-1 mb-2">Órdenes Cerradas ({ordenes.length})</h6>
                      {ordenes.length === 0 ? (
                        <p className="text-muted" style={{ fontSize: "0.88rem" }}>Sin órdenes cerradas</p>
                      ) : (
                        <ul className="list-group list-group-flush">
                          {ordenes.map((o) => (
                            <li key={o._id} className="list-group-item px-0 py-1 d-flex justify-content-between align-items-center" style={{ fontSize: "0.85rem" }}>
                              <div>
                                <span className="fw-semibold">{o.ordenServicio || "Sin folio"}</span>
                                <span className="text-muted ms-2">
                                  — {nombreClienteOrden(o)}
                                  {o.cliente?.esEmpleado && <span className="badge bg-warning text-dark ms-1">Empleado</span>}
                                </span>
