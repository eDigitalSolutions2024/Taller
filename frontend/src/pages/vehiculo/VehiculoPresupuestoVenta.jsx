...
export default function VehiculoPresupuestoVenta({ orden, onSaved, onGoPreparacion }) {
  ...
  // ── Empleados (para mostrar nombre en MO) ────────────────────────────────
  const [mecanicos, setMecanicos]   = useState([]);
  const [carroceros, setCarroceros] = useState([]);

  // ── BD Códigos (para factura) ─────────────────────────────────────────────
  ...
  // ── Mano de obra (solo lectura; se edita en Requisición y Diagnóstico) ───
  ...
  useEffect(() => {
    ...
    if (Array.isArray(orden.presupuesto) && orden.presupuesto.length > 0) {
      setPresRows(
        orden.presupuesto.map((p) => ({ ...p, autorizado: !!p.autorizado }))
      );
    } else {
      // Construir desde refacciones aprobadas con opción seleccionada
      const refParaPresupuesto = (orden.refaccionesSolicitadas || []).filter((r) => {
      ...
      setPresRows(
        refParaPresupuesto.map((r) => {
          ...
          return {
            ...
            moneda,
            tipoCambio,
            precioCompra:  precioMXN,
            tiempoEntrega: op.tiempoEntrega ?? "",
            horasMO:       0,
            precioVenta:   precioMXN,
  ...
  // Catálogo de servicios para factura
  useEffect(() => {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (serviciosDropdownRef.current && !serviciosDropdownRef.current.contains(e.target))
  ...
  const totalPresupuesto = useMemo(
    () => presRows.reduce((acc, r) => acc + Number(r.cant || 0) * Number(r.precioVenta || 0), 0),
    [presRows]
  );

  const totalVentaCliente = useMemo(
    () => ventaRows.reduce((acc, r) => acc + Number(r.cant || 0) * Number(r.precioVenta || 0), 0),
    [ventaRows]
  );
  ...
  const toggleAutorizado = (idx) => {
    if (isClosed) return;
    setPresRows((prev) =>
      prev.map((r, i) => (i === idx ? { ...r, autorizado: !r.autorizado } : r))
    );
  };
  ...
  const addManualPresRow = () => {
    ...
    setPresRows((prev) => [
      ...prev,
      { ...newPresLine, cant, precioCompra, precioVenta,
        horasMO: Number(newPresLine.horasMO) || 0, autorizado: false, manual: true },
    ]);
    setNewPresLine({
      cant: "", concepto: "", refaccion: "", tipo: "", marca: "",
      proveedor: "", codigo: "", precioCompra: "", moneda: "MN",
  ...
  // ── Enviar a Venta — cambia estado a PENDIENTE_SURTIR ────────────────────
  const handleEnviarAVenta = async () => {
    ...
    try {
      const res = await savePresupuestoVenta(
        orden._id,
        buildPayload({ presupuesto: presRows, ventaCliente: nuevasVentas, estadoOrden: "PENDIENTE_SURTIR" })
      );
      ...
      if (inv) {
        if (inv.pendientesSurtir === 0 && inv.autoSurtidas > 0) {
          ...
          alert(
            `${autorizadas.length} partida(s) enviada(s).\n` +
            `✅ ${inv.autoSurtidas} cubiertas desde inventario. ⏳ ${inv.pendientesSurtir} pendiente(s) de surtir.`
          );
        } else {
      ...
      <div className="card-body">
        ...
        <div className="table-responsive mb-2">
          <table className="table table-bordered table-sm align-middle">
            ...
            <tbody>
              ...
              {/* Fila azul de captura manual */}
              {!isClosed && (
                <tr className="table-info">
                  <td></td>
                  <td>
                    <input type="number" className="form-control form-control-sm" value={newPresLine.cant}
                      onChange={(e) => setNewPresLine({ ...newPresLine, cant: e.target.value })} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.concepto}
                      onChange={(e) => setNewPresLine({ ...newPresLine, concepto: e.target.value })} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.refaccion}
                      onChange={(e) => setNewPresLine({ ...newPresLine, refaccion: e.target.value })} />
                  </td>
                  <td>
                    <select className="form-select form-select-sm" value={newPresLine.tipo}
                      onChange={(e) => setNewPresLine({ ...newPresLine, tipo: e.target.value })}>
                  ...
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.marca}
                      onChange={(e) => setNewPresLine({ ...newPresLine, marca: e.target.value })} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.codigo}
                      onChange={(e) => setNewPresLine({ ...newPresLine, codigo: e.target.value })} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.proveedor}
                      onChange={(e) => setNewPresLine({ ...newPresLine, proveedor: e.target.value })} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.tiempoEntrega}
                      onChange={(e) => setNewPresLine({ ...newPresLine, tiempoEntrega: e.target.value })} />
                  </td>
                  <td>
                    <input type="number" className="form-control form-control-sm" value={newPresLine.precioCompra}
                      onChange={(e) => setNewPresLine({ ...newPresLine, precioCompra: e.target.value })} />
                  </td>
                  <td>
                    <select className="form-select form-select-sm" value={newPresLine.moneda || "MN"}
                      onChange={(e) => setNewPresLine({ ...newPresLine, moneda: e.target.value })}>
                  ...
                  <td>
                    {(newPresLine.moneda || "MN") === "USD" ? (
                      <input type="number" className="form-control form-control-sm" value={newPresLine.tipoCambio || ""}
                        onChange={(e) => setNewPresLine({ ...newPresLine, tipoCambio: e.target.value })} />
                    ) : (
                      <span className="text-muted">-</span>
                    )}
                  </td>
                  <td>
                    <input type="number" className="form-control form-control-sm" value={newPresLine.horasMO}
                      onChange={(e) => setNewPresLine({ ...newPresLine, horasMO: e.target.value })} />
                  </td>
                  <td>
                    <input type="number" className="form-control form-control-sm" value={newPresLine.precioVenta}
                      onChange={(e) => setNewPresLine({ ...newPresLine, precioVenta: e.target.value })} />
                  </td>
                  <td className="bg-info-subtle"></td>
                  <td>
                    <input type="text" className="form-control form-control-sm" value={newPresLine.observInt}
                      onChange={(e) => setNewPresLine({ ...newPresLine, observInt: e.target.value })} />
                  </td>
                  <td className="text-center">
                    <button type="button" className="btn btn-sm btn-danger fw-bold w-100" onClick={addManualPresRow}>
                      +
                    </button>
                  </td>
                </tr>
            ...
            <tfoot className="table-light">
              <tr>
                <td colSpan={14} className="text-end fw-bold text-uppercase">Total Presupuesto:</td>
        ...
        <div className="d-flex justify-content-end gap-2 mb-4">
          <button type="button" className="btn btn-outline-secondary btn-sm"
            disabled={isClosed} onClick={handleRegresarRefaccionaria}>
            Regresar a Refaccionaria
          </button>
          <button type="button" className="btn btn-danger btn-sm" onClick={handleImprimir}>
            Imprimir
          </button>
          <button type="button" className="btn btn-secondary btn-sm"
            disabled={isClosed} onClick={handleGuardarPresupuesto}>
            Guardar
          </button>
          <button type="button" className="btn btn-success btn-sm"
            disabled={isClosed} onClick={handleEnviarAVenta}
        ...
        {/* ===== VENTA AL CLIENTE ===== */}
        <h5 ref={ventaSectionRef} className="text-center mb-2 fw-bold">
        ...
        <div className="form-check mb-3">
          <input
            ...
            disabled={isClosed || ventaRows.length > 0}
                ...
                <tr>
                  <td>
                    <input type="number" className="form-control form-control-sm"
                      name="cant" value={ventaLine.cant} onChange={handleVentaLineChange} />
                  </td>

                  {requiereFactura && (
                    <td ref={serviciosDropdownRef} style={{ minWidth: 380, position: "relative" }}>
                      <input
                        className="form-control form-control-sm"
                      ...
                      {showServiciosDropdown && (
                        <div className="border bg-white shadow-sm"
                          style={{ position: "absolute", zIndex: 9999, top: 31, left: 0, right: 0,
                                   maxHeight: 170, overflowY: "auto", fontSize: 12 }}>
                          {serviciosFiltrados.length === 0 && (
                            <div className="px-2 py-2 text-muted">Sin resultados.</div>
                          )}
                          {serviciosFiltrados.map((srv) => (
                            <button key={srv._id} type="button"
                              className="btn btn-link w-100 text-decoration-none px-2 py-1 text-start"
                              style={{ fontSize: 12 }} onClick={() => seleccionarServicioVenta(srv)}>
                              <span className="fw-semibold text-primary">{srv.codigo} - {srv.descripcion}</span>
                              <span className="text-muted ms-2">SAT: {srv.codigoSat || "-"} - {srv.descripcionSat || "-"}</span>
                            </button>
                          ))}
                        </div>
                      )}
                  ...
                  <td>
                    <input type="text" className="form-control form-control-sm"
                      name="concepto" value={ventaLine.concepto} onChange={handleVentaLineChange} />
                  </td>
                  <td>
                    <input type="number" step="0.01" className="form-control form-control-sm"
                      name="precioVenta" value={ventaLine.precioVenta} onChange={handleVentaLineChange} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm"
                      name="observaciones" value={ventaLine.observaciones} onChange={handleVentaLineChange} />
                  </td>
                  <td className="text-center">
                ...
                <tr key={idx}>
                  <td className="text-center">
                    <input type="number" className="form-control form-control-sm text-center"
                      value={r.cant ?? ""} disabled={isClosed}
                      onChange={(e) => handleUpdateVentaRow(idx, "cant", e.target.value)} />
                  </td>
                  <td>
                    <input type="text" className="form-control form-control-sm"
                      value={r.concepto ?? ""} disabled={isClosed}
        ...
        <div className="d-flex justify-content-end gap-2 mb-4">
          <button type="button" className="btn btn-danger btn-sm"
            disabled={ventaRows.length === 0} onClick={handleImprimirVentaCliente}>
        ...
        <div className="row mb-3">
          <div className="col-md-6">
            ...
            <textarea className="form-control" rows={3} value={obsExternas}
              disabled={isClosed} onChange={(e) => setObsExternas(e.target.value)} />
          </div>
          <div className="col-md-6">
            <label className="form-label">Observaciones Internas:</label>
            <textarea className="form-control" rows={3} value={obsInternas}
              disabled={isClosed} onChange={(e) => setObsInternas(e.target.value)} />
          </div>
        </div>

        <div className="d-flex justify-content-end">
          <button type="button" className="btn btn-primary btn-sm"
            disabled={isClosed} onClick={handleGuardarOrdenServicio}>
