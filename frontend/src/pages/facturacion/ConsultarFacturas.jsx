...
export default function ConsultarFacturas() {
  ...
  const [pdfPreview, setPdfPreview] = useState(null); // { url, titulo }
  ...
  const verPdf = async (f) => {
    ...
    try {
      ...
      setPdfPreview({ url, titulo: `PDF — Folio ${folio || "—"}` });
    } catch (e) {
      console.error(e);
    ...
    <div className="container-fluid py-3" style={{ maxWidth: 1400 }}>
      ...
      {pdfPreview && (
        ...
        >
          ...
          >
            <div className="d-flex justify-content-between align-items-center p-2 border-bottom">
              <b>{pdfPreview.titulo}</b>
              <button className="btn btn-sm btn-outline-danger" onClick={cerrarPdfPreview}>
                Cerrar
              </button>
            </div>

            <iframe title="pdf" src={pdfPreview.url} style={{ width: "100%", height: "100%", border: 0 }} />
          </div>
        </div>
      )}
    </div>
